...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rightChild < max)
	{
		int minIdx = _smallerIndexHeap(heap, leftChild, rightChild);
		if (compare(heap->data[pos], heap->data[minIdx]) == 1)
		{
...
void sortHeap(DynArr *heap)
{
	...
	{
		for (j = 0; j < heap->size - i - 1; j++)
		{
			if (compare(heap->data[j], heap->data[j + 1]) == -1)
				swapDynArr(heap, j, j+1);
		}
	}
}
